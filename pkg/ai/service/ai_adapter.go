...
func (a *aiAdapter) buildFileParts(ctx context.Context, req *model.A2ARequest) []model.JSONRPCPart {
	...
	for i, att := range req.Attachments {
		if att.Data != "" {
			// ENIA: bytes already hydrated by the CRM (SendEventJob hydrates in a
			// separate job to avoid racing the ActiveStorage upload) — forward
			// inline, no download. Decoded size shares the byte budget with the
			// downloaded path so a burst cannot inflate the JSON-RPC body.
			decoded := base64.StdEncoding.DecodedLen(len(att.Data))
			if decoded > min(maxAttachmentBytes, remaining) {
