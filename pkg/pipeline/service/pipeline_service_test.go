...
type mockDispatchEngine struct {
	dispatchFn func(ctx context.Context, contactID, conversationID int64, content, audioURL, audioMime, audioData string, audioReplacesText bool, cfg model.BotConfig, postbackURL string) error
}

func (m *mockDispatchEngine) Dispatch(ctx context.Context, contactID, conversationID int64, content, audioURL, audioMime, audioData string, audioReplacesText bool, cfg model.BotConfig, postbackURL string) error {
	return m.dispatchFn(ctx, contactID, conversationID, content, audioURL, audioMime, audioData, audioReplacesText, cfg, postbackURL)
}
...
func setupSvcWithAI(t *testing.T, ai aiIface.AIAdapter) (*pipelineService, *redis.Client) {
	blockingDispatch := &mockDispatchEngine{
		dispatchFn: func(ctx context.Context, _, _ int64, _, _, _, _ string, _ bool, _ model.BotConfig, _ string) error {
			<-ctx.Done()
...
func TestPipeline_FullFlow_DispatchCompletes(t *testing.T) {
	...
	mockDispatch := &mockDispatchEngine{
		dispatchFn: func(_ context.Context, _, _ int64, _, _, _, _ string, _ bool, _ model.BotConfig, _ string) error {
			return nil
		},
	}
